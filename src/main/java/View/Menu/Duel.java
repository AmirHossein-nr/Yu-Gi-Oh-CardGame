...
public class Duel extends Menu {
    ...
    public void execute() {
        ...
        if (Regex.getMatcher(input, Regex.menuExit).find()) {
            ...
            this.execute();
        } else if ((matcher = Regex.getMatcher(input, Regex.newGameWithAi)).find()) {
            User user = new AI();
            if (Integer.parseInt(matcher.group(4)) != 1 && Integer.parseInt(matcher.group(4)) != 3) {
                System.out.println("number of rounds is not supported");
                this.execute();
            }
            int numberOfRounds = Integer.parseInt(matcher.group(4));
            new Game(loggedUser, user, numberOfRounds, scanner).run();
            this.execute();
        } else {
